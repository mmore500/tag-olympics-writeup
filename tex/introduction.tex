...
As instances of computer programs by their very nature, digital artificial life systems must also specify computational elements on which to act.
...
A querying operation's tag is compared to available operand tags. %fixed
...
These systems typically either use tagging schemes based on hamming distance between bitstrings (\textit{e.g.}, \citep{lalejini2018evolving} or \citep{banzhaf2003artificial}) or differences between real-valued scalars (\textit{e.g.}, \citep{riolo2001evolution} or \citep{spector2011tag}).
