...
During evaluation, we afford programs 64 time steps to execute the appropriate response instruction
...
[TODO - add settings description]
% - To select tag mutation rates for each metric, used a mutation sweep, selected most successful to
%   be used.
% - Outsource bulk of settings description to supplement.

\subsubsection{Directional-signal Task}

% Task overview
%The directional-signal task requires programs to adjust their response to environmental signals during
%evaluation.
...
% Evaluation overview
We evaluate programs on all possible sequences of four forward- and backward-signals (sixteen sequences).
