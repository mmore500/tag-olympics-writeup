...
What we're really interested is the ranking order of similarities over the $\cup_{i=0}^{32} \mathcal{E}^i \times \cup_{i=0}^{32} \mathcal{E}^i$ that the metric imposes, so applying a unifying transform onto the raw match distances helps to isolate that.
